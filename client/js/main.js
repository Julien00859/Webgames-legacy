    ...
    document.getElementById("select_game").addEventListener("submit", function(event){
        ...
        var app = playground({
            ...
            create: function() {
                ...
                this.loadImage("beam", "rock", "floor", "p_1_down", "p_1_left", "p_1_right", "p_1_up", "p_2_down", "p_2_left", "p_2_right", "p_2_up", "bomb", "exp_x", "exp_y", "exp_c", "B", "M", "P","S", "H"); //Incassable, Cassable, Sol
            ...
            render: function() {
                ...
                for (var explosion in explosions) {
                     ...
                     for (var i = explosions[explosion].position[0] - explosions[explosion].radius; i <= explosions[explosion].position[0] + explosions[explosion].radius; i++) {
                         //console.log(i);
                         //if (i <= 0) return;
                         this.layer.drawImage(this.images["exp_x"], i * 40 - 20, explosions[explosion].position[1] * 40 - 20, 40, 40);
                     }
                     ...
                     for (var i = explosions[explosion].position[1] - explosions[explosion].radius; i <= explosions[explosion].position[1] + explosions[explosion].radius; i++) {
                         //if (i <= 0) return;
                         this.layer.drawImage(this.images["exp_y"], explosions[explosion].position[0] * 40 - 20, i * 40 - 20, 40, 40);
                         this.layer.drawImage(this.images["exp_c"], explosions[explosion].position[0] * 40 - 20, explosions[explosion].position[1] * 40 - 20, 40, 40); // Center
                     }
